...
func returnError(err string) (string, int) {
	...
	jsonData, _ := json.Marshal(errorResponse) // Error convert in JSON string
...
func postHandler(args url.Values) (string, int) {
	...
	if arg, ok := args["in_park"]; ok {
		if len(arg[0]) > 255 {//if is too long
	...
	jsonData, _ := json.Marshal(idResponse) // Response convert in JSON string
...
// Write the entire header from the http request. It's define the header content (status code, method, content type)
